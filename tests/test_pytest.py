...
from vis_sim_numba import get_antenna_in_uvw
...
def test_beam():
    ras = np.array([0.0, 10], dtype=np.float64)
    decs = np.array([-27.0, -32], dtype=np.float64)
    fluxes = np.array([10, 20], dtype=np.float64)
    mid_frequency = 150e6

    attens, _ = compute_mwa_beam_attenuation(
        ras, decs, freq=mid_frequency, zenith_pointing=True
    )
    fluxes *= attens
    print(fluxes)
