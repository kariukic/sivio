...
ras = [0., 3.]  # generate_distribution(0., 4., N, "normal")
decs = [-27., -25]  # generate_distribution(-27., 4., N, "normal")
fluxes = [1, 1]  # np.abs(generate_distribution(1., 3., N, "normal"))
rdiffs = [1000]  # diffractive scales.


def run_vis(mset, ras, decs, fluxes, rdiffs, clean_vis=False, offset=True, sc=False, im=True):
    for rd in rdiffs:
        ...
        if im:
            ...
            datacol = 'DATA'
            command = 'wsclean -name %s -abs-mem 40 -size 2048 2048 -scale 25asec -niter 1000000 -auto-threshold 3 \
                        -data-column %s %s' % (imagename, datacol, mset)
            os.system(command)

            if sc:
                imagename = mset.split('.')[0] + '_%srd_scint' % (int(rd/1000))
                command2 = 'wsclean -name %s -abs-mem 40 -size 2048 2048 -scale 25asec -niter 1000000 -auto-threshold 3 \
                            -data-column %s %s' % (imagename, 'SCINT_DATA', mset)
                os.system(command2)
            if offset:
                imagename = mset.split('.')[0] + '_offsetvis'
                command3 = 'wsclean -name %s -abs-mem 40 -size 2048 2048 -scale 25asec -niter 1000000 -auto-threshold 3 \
                            -data-column %s %s' % (imagename, 'OFFSET_DATA', mset)
                os.system(command3)
...
if __name__ == "__main__":
    ...
    run_vis(mset, ras, decs, fluxes, rdiffs, clean_vis=True, offset=True, sc=False, im=True)
